        ...
        <div class="form-group">
            ...
            <select name="leerling_id" class="form-control">
                @foreach($leerlingen as $leerling)
                    <option value="{{ $leerling->id }}" {{ $les->leerling_id == $leerling->id ? 'selected' : '' }}>{{ $leerling->naam }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            <select name="instructeur_id" class="form-control">
                @foreach($instructeurs as $instructeur)
                    <option value="{{ $instructeur->id }}" {{ $les->instructeur_id == $instructeur->id ? 'selected' : '' }}>{{ $instructeur->naam }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            <select name="voertuig_id" class="form-control">
                @foreach($voertuigen as $voertuig)
                    <option value="{{ $voertuig->id }}" {{ $les->voertuig_id == $voertuig->id ? 'selected' : '' }}>{{ $voertuig->naam }}</option>
                @endforeach
            </select>
        </div>
        <button type="submit" class="btn btn-primary mt-3">Update Les</button>
    </form>
</div>
@endsection
